(define (domain coins)
  ...
  (:action step-in
    ...
    :precondition (and (located ?f ?p) (shaft ?e ?p))
    :effect (when (in ?e ?f) (and (inside ?e) (not (located ?f ?p))))
  )
  (:action step-out
    :parameters (?e - elevator ?f - floor ?p - pos)
    :precondition (and (inside ?e) (shaft ?e ?p))
    :effect (when (in ?e ?f) (and (located ?f ?p) (not (inside ?e))))
  )
  (:action move-left
    :parameters (?f - floor ?p ?np - pos)
    :precondition (and (located ?f ?p) (dec_p ?p ?np))
    :effect (and (not (located ?f ?p)) (located ?f ?np))
  )
  (:action move-right
    :parameters (?f - floor ?p ?np - pos)
    :precondition (and (located ?f ?p) (dec_p ?np ?p))
    :effect (and (not (located ?f ?p)) (located ?f ?np))
  )
  (:action collect
    :parameters (?c - coin ?f - floor ?p - pos)
    :precondition (located ?f ?p)
